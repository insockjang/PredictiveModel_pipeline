...
Data.type = c("Mh","C","E","EC","EMh","CMh","ECMh")#,"MhL","CL","EL","ECL","CMoL","EMoL","EMhL","CMhL","ECMoL","ECMhL")
    ...
    for(kk in k3){
      for(data.Type in Data.type){
        ...
        if(!file.exists(filename)){
          resultsScale <- myBSModel1(kk,data.set = "CCLE",data.type=data.Type, drug.type = drug.Type, model.type = model.Type, numBS= 100, numCore = 5)    
