myModel<-function(kk,
  ...
  myRidge<-function(X,Y){
    source("~/PredictiveModel_pipeline/R5/myEnetModel1.R")    
    CV<-crossValidatePredictiveModel1(X, Y, model = myEnetModel1$new(), alpha = 0, numFolds = nfolds)
    return(CV)
  }
  myRF<-function(X,Y){
  ...
  filteredData<-filterPredictiveModelData(dataSet$featureData,dataSet$responseData[,kk,drop=FALSE],featureVarianceThreshold = 0.2)
